...
if ($action == "highest_price") {
    $stmt = $pdo->prepare("SELECT * FROM products where field='painting' ORDER BY price DESC");
    ($stmt)->execute();
    $products = $stmt->fetchAll(PDO::FETCH_ASSOC);
    

} elseif ($action == "lowest_price") {
    $stmt = $pdo->prepare("SELECT * FROM products where field='painting' ORDER BY price ASC");
    ($stmt)->execute();
    $products = $stmt->fetchAll(PDO::FETCH_ASSOC);
} elseif ($action == "top_rated") {
  $stmt = $pdo->prepare("SELECT * FROM products where field='painting' ORDER BY rate DESC");
    ($stmt)->execute();
    $products = $stmt->fetchAll(PDO::FETCH_ASSOC);
} elseif ($action == "best_seller") {

} else {
    $stmt = $pdo->prepare("SELECT * FROM products where field='painting'");
    $stmt->execute();
   $products =$stmt->fetchAll(PDO::FETCH_ASSOC);
}
        ...
        <div class="container">
            <div class="nav-bar">
                <ul class="links">
                    ...
                    <li>
                        <a href="./painting.php" class="cool-link active"></i>نقاشة</a>
                    </li>
                    <li>
            ...
            <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight"
                ...
                <div class="offcanvas-body">
                    <div class="d-flex-column">
                        <?php echo "<a href='painting.php?action=all'><button class=' btn button__value' >الكل </button></a>"; ?>
                        <br>
                        <?php echo "<a href='painting.php?action=best_seller'><button class=' btn button__value' >الاكثر مبيعاً </button></a>"; ?>
                        <br>
                        <?php echo "<a href='painting.php?action=top_rated'><button class=' btn button__value' >الاعلى تقييماً</button></a>"; ?>
                        <br>
                        <?php echo "<a href='painting.php?action=highest_price'><button class=' btn button__value' >الاعلى سعراً</button></a>"; ?>
                        <br>
                        <?php echo "<a href='painting.php?action=lowest_price'><button class=' btn button__value' >الاقل سعراً</button></a>"; ?>
                    </div>
                </div>
            </div>
        </div>
            ...
            foreach ($products as $value): ?>
                <div class='card mt-5 '>
                    <div class='position-relative'>
                        ...
                        <div class='btns'>
                            <?php echo "<a href= '../index.php?add_to_cart=$value[id]&view=painting'><button class='cart_btn tooltip'><span
                                        class='tooltiptext'>اضف الى السلة</span><i
                                        class='fa fa-cart-plus'></i></button></a>"; ?>

                            <?php echo "<a href= '../index.php?fav=$value[id]&view=painting'> <button class='fav_btn tooltip'><span class='tooltiptext'>اضف للمفضلة</span><i
                    ...
                    <div class='card-body'>
                        ...
                        <p class='card-text '>
                            ...
                            <?php endif; ?>
                            <?php echo " <a href='painting.php?add_to_cart=$value[id]'> <button class='btns btn-outline-success card_btn'><i class='fa-solid fa-cart-plus'></i></button></a>"; ?>
