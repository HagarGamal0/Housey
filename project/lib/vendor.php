...
class vendor{
    ...
    public function vendorRegistration($data){
	  ...
	  move_uploaded_file($_FILES["image"]["tmp_name"],"./assets/img/". $im_name);
        ...
        if($name ==  "" OR $store_name ==  "" OR $email ==  "" OR $password ==  "" OR $cpassword ==  ""OR $tax_number ==  "" OR $address ==  "" OR $phone ==  "" ){
        ...
        $query = "INSERT INTO `vendors`(name,store_name, password,email,image,tax_number,address,phone,field) VALUES (:name,:store_name,:password_hash,:email,:im_name,:tax_number,:address,:phone,:field)";
    ...
    public function vendorLogin($data){
        ...
        if($result){
            ...
            session::set("field", $result->field);
        ...
        if ($result && password_verify($password_hash, $result['password'] ))
{
    ...
    public function vendorUpdate($id, $data){
      ...
      $emailcheck = $this->checkEmail($email);
        ...
        ,'phone'=:phone ,'field'=:field   WHERE `id` = :id";
        $sql = $this->db->prepare($query);
        $sql->bindValue(':name', $name);
        $sql->bindValue(':store_name', $store_name);
